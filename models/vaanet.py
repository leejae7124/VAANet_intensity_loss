...
class VAANet(VisualStream):
    ...
    def forward(self, visual: torch.Tensor, audio: torch.Tensor, saliency_map: torch.Tensor):
        ...
        if self.saliency_level == 'input':
            ...
            saliency_map = saliency_map.transpose(0, 1)  # [Seq, B, 1, D, H, W]
            seq_len, batch_size = saliency_map.shape[0], saliency_map.shape[1]

            # [Seq, B, 1, D, H, W] → [Seq * B, 1, D, H, W]
            saliency_map_flat = saliency_map.reshape(seq_len * batch_size, 1, *saliency_map.shape[3:])

            # AdaptiveAvgPool3d는 5D 입력만 받음
            saliency_resizer = nn.AdaptiveAvgPool3d((snippet_duration, sample_size, sample_size))
            saliency_map_resized = saliency_resizer(saliency_map_flat)

            # 다시 원래 shape로 복원: [Seq * B, 1, D, H, W] → [Seq, B, 1, D, H, W]
            saliency_map_resized = saliency_map_resized.view(seq_len, batch_size, 1, snippet_duration, sample_size, sample_size)

            # visual: [Seq, B, C, D, H, W]
            saliency_mask = saliency_map_resized.expand_as(visual)  # [Seq, B, C, D, H, W]

            # # 곱해서 gate 적용
            # visual = visual * saliency_mask
