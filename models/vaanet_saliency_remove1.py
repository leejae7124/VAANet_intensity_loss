...
# (1) 기존 spatial attention 제거 후, ResNet 직후 Saliency attention

class VAANet_remove1(VisualStream):
    def __init__(self,
                 snippet_duration=16,
                 sample_size=112,
                 n_classes=8,
        ...
        super(VAANet_remove1, self).__init__(
    ...
    def forward(self, visual: torch.Tensor, audio: torch.Tensor, saliency_map: torch.Tensor):
        ...
        # ▶ 교체 코드: 공간 어텐션 제거(평균=1인 균등 가중)
        As = torch.ones(F.size(0), self.hp['m'], device=F.device, dtype=F.dtype)  # [seq_len*batch, m]
        alpha = As.view(seq_len, batch, self.hp['m'])  # 인터페이스 유지용
        fS = F  # 곱해도 동일하니 그냥 패스
