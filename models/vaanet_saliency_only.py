...
#중복 공간 어텐션을 제거하고, 내가 적용한 가중치 방식만 적용.

class VAANet_saliency_only(VisualStream):
    def __init__(self,
                 snippet_duration=16,
                 sample_size=112,
                 n_classes=8,
        ...
        super(VAANet_saliency_only, self).__init__(
    ...
    def forward(self, visual: torch.Tensor, audio: torch.Tensor, saliency_map: torch.Tensor):
        ...
        #공간 어텐션 계산
        # Hs = self.sa_net['conv'](F) #각 spatial 위치(16개)에 대한 어텐션 스코어를 만듦
        # Hs = torch.squeeze(Hs, dim=1) #차원 정리
        # Hs = self.sa_net['fc'](Hs)
        # As = self.sa_net['softmax'](Hs) #m개의 위치에 대해 softmax -> 공간 어텐션 분포 (합이 1)
        # As = torch.mul(As, self.hp['m'])
        # alpha = As.view(seq_len, batch, self.hp['m'])

        # --- 기존 sa_net spatial attention 블록 제거/주석 처리 후, 아래로 대체 ---
        
        sb = seq_len * batch
        m = self.hp['m']
        
        # 1) alpha는 (원하면) 균일 attention으로 대체: 모든 위치 가중치=1 (합=m)
        As = torch.ones(sb, m, device=F.device, dtype=F.dtype)
        alpha = As.view(seq_len, batch, m)
        
        # 2) 공간 어텐션 적용을 없앰: fS는 그냥 F로 통과
        fS = F


        # fS = torch.mul(F, torch.unsqueeze(As, dim=1).repeat(1, self.hp['k'], 1)) #만들어진 공간 어텐션 적용!!
