...
class VisualStream(nn.Module):
    def __init__(self,
        ...
        if self.saliency_level == 'input':
            self.saliency_resizer = nn.AdaptiveAvgPool3d((snippet_duration, sample_size, sample_size))
        elif self.saliency_level == 'feature_map':
            self.saliency_resizer = nn.AdaptiveAvgPool3d((1, self.hp['hw'], self.hp['hw']))
    ...
    def _init_encoder(self): #인코더(백본) 정의
    ...
    def forward(self, input: torch.Tensor, saliency_map: torch.Tensor):
        ...
        if self.saliency_level == 'input':
            saliency_map_expanded = saliency_map.repeat(1, 1, nc, 1, 1, 1)

            mask = 1 + 0.5 * saliency_map_expanded
        ...
        output = self.fc(fSCT)#베타: 시각 채널별 주의 맵, 감마: 시각 시간적 주의 맵
        return output, alpha, beta, gamma #alpha: 공간 해상도를 1차원으로 펼친 값. 시각 공간 주의 맵.