...
def make_dataset(frame_root_path,
    ...
    for class_name in sorted(os.listdir(subset_frame_root)):
        ...
        for video_id in sorted(os.listdir(class_dir)):
            ...
            # n_frames 파일이 있으면 읽고, 없으면 jpg 개수로 계산
...
class CAERDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if index < 3:
            if hasattr(self.spatial_transform, "f1_1") and hasattr(self.saliency_transform, "f1_1"):
                print(
                    "[crop check]",
                    "rgb_crop=", getattr(self.spatial_transform.f1_1, "crop_position", None),
                    "sal_crop=", getattr(self.saliency_transform.f1_1, "crop_position", None),
                )

            if hasattr(self.spatial_transform, "f2") and hasattr(self.saliency_transform, "f2"):
                print(
                    "[aug check]",
                    "rgb_p=", getattr(self.spatial_transform.f2, "p", None),
                    "sal_p=", getattr(self.saliency_transform.f2, "p", None),
                )
        ...
        for snippet_frame_idx in snippets_frame_idx:
            ...
            snippet = torch.stack(snippet, 0).permute(1, 0, 2, 3)   # [C, T, H, W]
            ...
            if index < 3:
                print(
                    ...
                    saliency_snippet.min().item(),
                    saliency_snippet.max().item(),
                    saliency_snippet.mean().item()
                )

            saliency_snippets.append(saliency_snippet)

        snippets = torch.stack(snippets, 0)                   # [num_snip, C, T, H, W]
        saliency_snippets = torch.stack(saliency_snippets, 0) # [num_snip, 1, T, H, W]
